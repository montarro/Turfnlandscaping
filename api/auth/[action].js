/* /api/auth/login | /api/auth/logout | /api/auth/me — one function for all
   three endpoints (Hobby plan allows max 12 serverless functions per deploy).

   login: verifies credentials against Supabase Auth, enforces the owner-email
   allowlist and per-IP rate limiting, then sets the HTTP-only session cookie.
   No passwords are stored anywhere in this codebase.
   logout: clears the session cookie.
   me: reports the signed-in admin, or a diagnosable 401. */
const { allowedEmails, setSession, clearSession, getSessionDebug, checkRateLimit, recordFailure, clearFailures } = require("../_lib/auth");
